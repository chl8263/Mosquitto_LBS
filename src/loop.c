...
{
#ifdef WITH_SYS_TREE
	time_t start_time = mosquitto_time();	//시작된 시간을 알려줌
#endif
#ifdef WITH_PERSISTENCE
	...
	if(db->config->persistent_client_expiration > 0){
		expiration_check_time = time(NULL) + 3600;	//만료시간 ??
	}

	while(run){
		printf("------------------------start_loop-------------------\n");
		mosquitto__free_disused_contexts(db);	// 안쓰는 context 는 메모리에서 해지 한다.
#ifdef WITH_SYS_TREE
		...
		memset(pollfds, -1, sizeof(struct pollfd)*pollfd_max);	// 안쓰는 값인 -1 로 초기화를 한다,

		pollfd_index = 0;
		for(i=0; i<listensock_count; i++){
			printf("listensock_count %d\n\n",listensock_count);
		...
		HASH_ITER(hh_sock, db->contexts_by_sock, context, ctxt_tmp){	
			...
			if(context->sock != INVALID_SOCKET){
				...
				if(!(context->keepalive)
					...
					if(mqtt3_db_message_write(db, context) == MOSQ_ERR_SUCCESS){	//mosquitto 메모리 db에 값을 적는 부분, 성공했을때

						printf("pollfdindex->%d\n",pollfd_index);
...
#else
		fdcount = WSAPoll(pollfds, pollfd_index, 100);		//소켓의 상태를 판정하는 window Api
#endif
		if(fdcount == -1){
			_mosquitto_log_printf(NULL, MOSQ_LOG_ERR, "Error in poll: %s.", strerror(errno));
		}else{
			loop_handle_reads_writes(db, pollfds);	// 네트워크 통신이 이루어짐 모든 네트워크 통신	밑에서 만들어진 context 를 hashiter로 탐색

			for(i=0; i<listensock_count; i++){
				if(pollfds[i].revents & (POLLIN | POLLPRI)){
					while(mqtt3_socket_accept(db, listensock[i]) != -1){	// 처음 들어온 소켓을 처리하는 부분 context 로 만들어 주는곳
					}
				}
			}
		}
...
static void loop_handle_reads_writes(struct mosquitto_db *db, struct pollfd *pollfds)
{
	...
	HASH_ITER(hh_sock, db->contexts_by_sock, context, ctxt_tmp){
		...
		assert(pollfds[context->pollfd_index].fd == context->sock);		// null 인지 판별해 주는곳
		...
		if(pollfds[context->pollfd_index].revents & POLLOUT){
#endif
			if(context->state == mosq_cs_connect_pending){		//커넥트 상태일때 ??
			...
			if(_mosquitto_packet_write(context)){		//패킷을 보내야 하는 형태로 바꾼다 ??
				do_disconnect(db, context);
				continue;
			}
		}
...
#endif
			do{
				if(_mosquitto_packet_read(db, context)){		//실질적인 메세지 읽는 부분	(lib net_mosq.c 에 있음)0
					do_disconnect(db, context);
					continue;
				}
