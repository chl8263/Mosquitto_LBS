...
int mqtt3_db_open(struct mqtt3_config *config, struct mosquitto_db *db)
{
	...
	child->parent = NULL;
	child->next = NULL;
	child->topic = _mosquitto_strdup("");		//빈 구독 트리 하나 생성
	if(!child->topic){
		_mosquitto_log_printf(NULL, MOSQ_LOG_ERR, "Error: Out of memory.");
		return MOSQ_ERR_NOMEM;
	}
	...
	child->parent = NULL;
	child->next = NULL;
	child->topic = _mosquitto_strdup("$SYS");	//sys 하나 생성
	if(!child->topic){
		_mosquitto_log_printf(NULL, MOSQ_LOG_ERR, "Error: Out of memory.");
		return MOSQ_ERR_NOMEM;
	}
...
int mqtt3_db_message_store_find(struct mosquitto *context, uint16_t mid, struct mosquitto_msg_store **stored)
{
	...
	*stored = NULL;
	tail = context->msgs;
	//printf("mqtt3_db_message_store_find %s\n",context->msgs->store->topic);
	while(tail){
...
int mqtt3_db_message_write(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	if(context->state != mosq_cs_connected){ //client 의 연결이 있을때
		printf("1\n");
		//printf("id->%s\n",context->hh_id);
		Sleep(1000);
		return MOSQ_ERR_SUCCESS;
	...
	tail = context->msgs;	//메세지 가져오는 부분 , 한개의 context에 모든 메세지를 연결리스트로 저장함

	//Sleep(1000);

	while(tail){		//여기는 전부 subscribe 들임
		if(tail->direction == mosq_md_in){
			msg_count++;
			//printf("%d\n",&msg_count);
		}
		if(tail->state != mosq_ms_queued){		//메세지를 전부 분리함
			mid = tail->mid;
			retries = tail->dup;
			retain = tail->retain;
			topic = tail->store->topic;
			...
			switch(tail->state){
				case mosq_ms_publish_qos0:
					...
					if(!rc){
						_message_remove(db, context, &tail, last);	//tail 을 지운다 . 메세지는 단발성 이기때문
					}else{
						return rc;
					}
					break;
				...
				case mosq_ms_publish_qos1:
					...
					last = tail;		// qos 1번은 여러번 보내니가 다시 달아줌
					tail = tail->next;
					break;
