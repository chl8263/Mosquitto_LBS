...
int mqtt3_handle_subscribe(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	while(context->in_packet.pos < context->in_packet.remaining_length){
		...
		if(sub){
			...
			if(mosquitto_sub_topic_check(sub)){		//토픽이 어떻게 분할 되는지 알아냄 / , + 등등...
			...
			if(context->listener && context->listener->mount_point){		//여기가 트리구조를 가지고 노는곳 같은데 잘 모르겠음 ..????
			...
			if(qos != 0x80){
				...
				rc2 = mqtt3_sub_add(db, context, sub, qos, &db->subs);	// 트리에 
				if(rc2 == MOSQ_ERR_SUCCESS){
					if(mqtt3_retain_queue(db, context, sub, qos)) rc = 1;	// 값을 저장하여 나중에 들어와도 될수 있도록 조짐
				}else if(rc2 != -1){
					rc = rc2;
				}
