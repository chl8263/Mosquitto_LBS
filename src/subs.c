...
static int _subs_process(struct mosquitto_db *db, struct _mosquitto_subhier *hier, const char *source_id, const char *topic, int qos, int retain, struct mosquitto_msg_store *stored, bool set_retain)
{
	...
	while(source_id && leaf){
		...
		if(rc2 == MOSQ_ERR_ACL_DENIED){
			...
			if(mqtt3_db_message_insert(db, leaf->context, mid, mosq_md_out, msg_qos, client_retain, stored) == 1) rc = 1;		//여기에서 씀
		}else{
	...
	/* FIXME - this function has the potential to leak subhier, audit calling functions. */
{
	...
	if(!tokens){
		...
		if(context && context->id){		// subscribe id 를 가지고 논다.
			...
			leaf = _mosquitto_malloc(sizeof(struct _mosquitto_subleaf));	//leaf 구조체를 하나 만든다
			...
			for(i=0; i<context->sub_count; i++){
				if(!context->subs[i]){
					context->subs[i] = subhier;		//		context 는 해당 가지를 가지고 있다
					break;
				}
			}	
...
static void _sub_search(struct mosquitto_db *db, struct _mosquitto_subhier *subhier, struct _sub_token *tokens, const char *source_id, const char *topic, int qos, int retain, struct mosquitto_msg_store *stored, bool set_retain)
{
	...
	while(branch){
		...
		if(tokens && tokens->topic && (!strcmp(branch->topic, tokens->topic) || !strcmp(branch->topic, "+"))){
			...
			if(!tokens->next){
				_subs_process(db, branch, source_id, topic, qos, retain, stored, sr);		//여기안에서 write 함
			}
...
int mqtt3_sub_add(struct mosquitto_db *db, struct mosquitto *context, const char *sub, int qos, struct _mosquitto_subhier *root)
{
	...
	while(subhier){
		if(!strcmp(subhier->topic, tokens->topic)){
			//printf("***************%s %s %s***********\n", tokens->next->topic, sub, subhier->topic);
			rc = _sub_add(db, context, qos, subhier, tokens);		//여기에서 해당 노드에 subscribe context 를 추가함
			break;
		}
...
{
	int rc = 0;
	struct _mosquitto_subhier *subhier;		//하나의 트리 구조채를 선언
	struct _sub_token *tokens = NULL;

	assert(db);
	assert(topic);

	if(_sub_topic_tokenise(topic, &tokens)) return 1;	//topic 의 구조를 자르는곳 ex) '/'
	...
	while(subhier){
		if(!strcmp(subhier->topic, tokens->topic)){
			if(retain){
				 ...
				 */
				_sub_add(db, NULL, 0, subhier, tokens);		// 트리에 달아줌
			}
			_sub_search(db, subhier, tokens, source_id, topic, qos, retain, *stored, true);		// 여기 들어감
		}
		subhier = subhier->next;
	}
	_sub_topic_tokens_free(tokens);
