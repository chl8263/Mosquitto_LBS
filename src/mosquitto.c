...
int main(int argc, char *argv[])
{
	...
	rc = mqtt3_db_open(&config, &int_db);	// 구독 빈 트리를 하나 생성함
	...
	for(i=0; i<config.listener_count; i++){
		if(config.listeners[i].protocol == mp_mqtt){
			...
			listensock_count += config.listeners[i].sock_count;		//config 의 listener 의 socket 갯수
			listensock = _mosquitto_realloc(listensock, sizeof(mosq_sock_t)*listensock_count);	//realloc 만큼 사이즈를 늘려줌
			if(!listensock){
				mqtt3_db_close(&int_db);
				if(config.pid_file){
					remove(config.pid_file);
	...
	rc = mosquitto_main_loop(&int_db, listensock, listensock_count, listener_max);	// ========================   Loop   =========================
