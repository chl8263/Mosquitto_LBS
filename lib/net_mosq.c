...
int _mosquitto_read_string(struct _mosquitto_packet *packet, char **str)
{
	...
	assert(packet);
	rc = _mosquitto_read_uint16(packet, &len);		//패킷의 payload 에 길이를 확보한다
	if(rc) return rc;

	if(packet->pos+len > packet->remaining_length) return MOSQ_ERR_PROTOCOL;	//많으면 중지

	*str = _mosquitto_malloc(len+1);
	if(*str){
		memcpy(*str, &(packet->payload[packet->pos]), len);		//패킷의 payload 의 메세지를 복사한다.
		(*str)[len] = '\0';
		packet->pos += len;
	}else{
		return MOSQ_ERR_NOMEM;
...
int _mosquitto_packet_write(struct mosquitto *mosq)
{
	...
	pthread_mutex_lock(&mosq->current_out_packet_mutex);	//여기서 상호배제를 왜 하는지...
	pthread_mutex_lock(&mosq->out_packet_mutex);
...
#endif
{
	...
	if(!mosq->in_packet.command){
		read_length = _mosquitto_net_read(mosq, &byte, 1);		// poll 내부 버퍼에서 읽는 부분 --> pub , sub ack 인지 알 수 있다. fixed header
		if(read_length == 1){
...
#  endif
	 ...
	 */
	if(mosq->in_packet.remaining_count <= 0){		// 남은 부분 읽는 부분	fixed header
		do{
	...
	while(mosq->in_packet.to_process>0){		//pay load 읽는부분	
